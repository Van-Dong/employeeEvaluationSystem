...
class RankingServiceTest {
    ...
    void getEmployeeScore() {
        ...
        List<Employee> employeeList = List.of(
                Employee.builder().id(1L).fullName("Nguyen Van A").build(),
        ...
        Page<EmployeeScore> employeeScorePage =
                new PageImpl<>(employeeScoreList, PageRequest.of(page, 10), employeeScoreList.size());

        Mockito.when(evaluationRepository.personalAchievementsByEmployeeName(
                        "%" + searchName + "%", PageRequest.of(page, 10)))
        ...
        Assertions.assertSame(
                employeeScorePage.getContent().get(0).getEmployee().getId(),
                employeeScorePage.getContent().get(0).getId());
    }
    ...
    void getDepartmentScore() {
        ...
        Mockito.verify(evaluationRepository, Mockito.times(1))
                .departmentAchievementsByNameLike("%" + searchName + "%", PageRequest.of(page, 10));
    }
}
